...
import { GENERAL_QUESTIONS , ABOUT_CLIENT_WORK } from '@/config/GeneralConfigH_11'
    ...
    <Accordion type='single' collapsible className='w-full' defaultValue='item-1'>
      ...
      {GENERAL_QUESTIONS.map((item, index) => (
        <AccordionItem key={index} value={`item-${index + 1}`}>
          ...
          <AccordionContent className='font-mono text-xs text-muted-foreground'>{item.content}</AccordionContent>
        </AccordionItem>
      ))}
    </Accordion>
  )
...
export const FAQ_B = () => {
    return (
      <Accordion type='single' collapsible className='w-full' defaultValue='item-1'>
        ...
        {ABOUT_CLIENT_WORK.map((item, index) => (
          <AccordionItem key={index} value={`item-${index + 1}`}>
            <AccordionPrimitive.Header className='flex items-center gap-2'>
              <AccordionPrimitive.Trigger
                data-slot='accordion-trigger'
...
export const FAQ = () => {
    return (
        <div className='w-full h-full lg:pt-10 pb-10 px-2 lg:px-8'>
            <div className='w-full mx-auto flex flex-col items-center justify-center pt-16 lg:pb-10'>
                <h1 className='text-center font-mono font-bold text-3xl lg:text-4xl text-neutral-800 dark:text-neutral-200'>Ask me anything.</h1>
            </div>
